...
async function main(): Promise<void> {
  ...
  const reminded = await service.remindDueEditors();
  console.log(JSON.stringify({ reminded: reminded.map((schedule) => schedule.number) }));
}

main().catch((error: unknown) => { console.error(error instanceof Error ? error.message : 'Editing reminder batch failed'); process.exitCode = 1; });
